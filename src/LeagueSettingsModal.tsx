...
export function LeagueSettingsModal({ isOpen, onClose, leagueId, currentName, currentLockDate, currentScoringType, onUpdate }: Props) {
		...
		<div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
			...
			<div className="relative bg-[#0F172A] border border-gray-700 w-full max-w-md rounded-2xl shadow-2xl p-6 overflow-hidden">
				...
				<div className="space-y-6">
					{/* League Name */}
					<div>
						<label className="block text-blue-200 text-xs font-bold uppercase mb-2">League Name</label>
						<input 
							type="text" 
							value={name}
					...
					<div>
						<label className="block text-blue-200 text-xs font-bold uppercase mb-2">Scoring System</label>
						<div className="grid grid-cols-2 gap-3">
							<button
								...
								>
									Standard
									<span className="block text-[10px] font-normal opacity-70 mt-1">Linear Penalty (Abs Diff)</span>
								</button>
								...
								>
									Strict (Squared)
									<span className="block text-[10px] font-normal opacity-70 mt-1">Heavy Penalty for big misses</span>
								</button>
							</div>
						</div>
					...
					<div className="flex gap-3 mt-8 pt-4 border-t border-gray-800">
						<button onClick={onClose} className="flex-1 py-3 rounded-lg text-gray-400 hover:bg-white/5">Cancel</button>
						<button 
							onClick={handleSave} 
							disabled={loading}
