...
// --- 1. Dependency map (same as before) ---
...
// --- Helper for recursive cleanup (external so it can be used during loading) ---
...
const BracketLines = ({ type, isRev = false }: { type: 'r1_to_semis' | 'semis_to_finals', isRev?: boolean }) => {
    ...
    if (type === 'r1_to_semis') {
        return (
            <div className="w-[30px] flex-shrink-0 h-full relative z-0"> 
                <svg width="100%" height="100%" style={{ transform: isRev ? 'scaleX(-1)' : 'none' }}>
                    <path d="M0,62.5 H15 V187.5 H0" fill="none" stroke={stroke} strokeWidth={strokeWidth} />
                    <path d="M15,125 H30" fill="none" stroke={stroke} strokeWidth={strokeWidth} />
                    <path d="M0,312.5 H15 V437.5 H0" fill="none" stroke={stroke} strokeWidth={strokeWidth} />
    ...
    if (type === 'semis_to_finals') {
        return (
             <div className="w-[30px] flex-shrink-0 h-full relative z-0">
                <svg width="100%" height="100%" style={{ transform: isRev ? 'scaleX(-1)' : 'none' }}>
                    <path d="M0,125 H15 V375 H0" fill="none" stroke={stroke} strokeWidth={strokeWidth} />
...
export function FullPlayoffBracket({ userId, leagueId, isLocked }: Props) {
  ...
  useEffect(() => {
    ...
    async function loadData() {
        ...
        if (rankData) {
            const west = rankData.filter(p => p.conference === 'West').map(p => NBA_TEAMS.find(t => t.id === p.team_id)!);
            const east = rankData.filter(p => p.conference === 'East').map(p => NBA_TEAMS.find(t => t.id === p.team_id)!);
            setStandings({ West: west, East: east });
        }

        // 2. Load picks (just load what's there, no heavy validation)
        ...
        if (bracketData && bracketData.length > 0) {
            ...
            setPicks({}); // if empty (trigger deleted), start empty
        }
  ...
  const resolveSlot = (conf: 'West' | 'East', seed: number | string, matchSource?: string): Team | null => {
  ...
  const MainBracketSide = ({ conf }: { conf: 'West' | 'East' }) => {
      ...
      return (
        <div className={`flex ${isRev ? 'flex-row-reverse' : 'flex-row'} items-center h-[500px]`}>
            
            <div className="flex flex-col justify-around h-full w-40 flex-shrink-0 z-10">
                 <BracketMatch 
                    matchId={`${p}_r1_1`} topTeam={resolveSlot(conf, 1)} bottomTeam={final8thSeed} topSeed={1} bottomSeed={8}
                    onWinnerClick={handleWin} selectedWinnerId={picks[`${p}_r1_1`]} reverse={isRev}
                    officialWinnerId={getOfficial(`${p}_r1_1`)}
                />
                <BracketMatch 
                    matchId={`${p}_r1_4`} topTeam={resolveSlot(conf, 4)} bottomTeam={resolveSlot(conf, 5)} topSeed={4} bottomSeed={5}
                    onWinnerClick={handleWin} selectedWinnerId={picks[`${p}_r1_4`]} reverse={isRev}
                    officialWinnerId={getOfficial(`${p}_r1_4`)}
                />
                <BracketMatch 
                    matchId={`${p}_r1_3`} topTeam={resolveSlot(conf, 3)} bottomTeam={resolveSlot(conf, 6)} topSeed={3} bottomSeed={6}
                    onWinnerClick={handleWin} selectedWinnerId={picks[`${p}_r1_3`]} reverse={isRev}
                    officialWinnerId={getOfficial(`${p}_r1_3`)}
                />
                 <BracketMatch 
                    matchId={`${p}_r1_2`} topTeam={resolveSlot(conf, 2)} bottomTeam={winner7v8} topSeed={2} bottomSeed={7}
                    onWinnerClick={handleWin} selectedWinnerId={picks[`${p}_r1_2`]} reverse={isRev}
            ...
            <div className="flex flex-col justify-around h-full w-40 flex-shrink-0 z-10">
                <BracketMatch 
                    matchId={`${p}_semis_1`} topTeam={getTeam(picks[`${p}_r1_1`])} bottomTeam={getTeam(picks[`${p}_r1_4`])} 
                    onWinnerClick={handleWin} selectedWinnerId={picks[`${p}_semis_1`]} reverse={isRev}
                    officialWinnerId={getOfficial(`${p}_semis_1`)}
                />
                 <BracketMatch 
                    matchId={`${p}_semis_2`} topTeam={getTeam(picks[`${p}_r1_3`])} bottomTeam={getTeam(picks[`${p}_r1_2`])} 
                    onWinnerClick={handleWin} selectedWinnerId={picks[`${p}_semis_2`]} reverse={isRev}
            ...
            <div className="flex flex-col justify-center h-full w-40 flex-shrink-0 z-10">
                 <div className={`text-[9px] font-bold mb-2 text-center uppercase tracking-widest ${conf === 'West' ? 'text-blue-400' : 'text-red-400'}`}>{conf} Final</div>
                 <BracketMatch 
                    matchId={`${p}_finals`} topTeam={getTeam(picks[`${p}_semis_1`])} bottomTeam={getTeam(picks[`${p}_semis_2`])} 
                    onWinnerClick={handleWin} selectedWinnerId={picks[`${p}_finals`]} reverse={isRev}
  ...
  const PlayInBracket = ({ conf }: { conf: 'West' | 'East' }) => {
      const p = conf === 'West' ? 'w' : 'e';
      const colorClass = conf === 'West' ? 'text-blue-400 border-blue-900/30' : 'text-red-400 border-red-900/30';
      ...
      return (
          <div className={`bg-black/20 border ${colorClass} rounded-lg p-3 relative min-w-[380px]`}>
              <div className={`absolute top-0 left-0 bg-black/60 px-2 py-0.5 text-[8px] font-bold rounded-br border-b border-r border-white/10 ${conf === 'West' ? 'text-blue-400' : 'text-red-400'}`}>
                  {conf} PLAY-IN
              </div>
              ...
              <div className="flex items-center gap-6 mt-3">
                  <div className="flex flex-col gap-3">
                      <div className="relative">
                          <div className="text-[7px] uppercase text-gray-500 mb-0.5 ml-1 font-bold">7 vs 8</div>
                          <BracketMatch 
                              matchId={pi_7v8_id} topTeam={team7} bottomTeam={team8} topSeed={7} bottomSeed={8}
                              onWinnerClick={handleWin} selectedWinnerId={picks[pi_7v8_id]}
                              officialWinnerId={getOfficial(pi_7v8_id)}
                          />
                      </div>
                      <div className="relative">
                          <div className="text-[7px] uppercase text-gray-500 mb-0.5 ml-1 font-bold">9 vs 10</div>
                          <BracketMatch 
                              matchId={pi_9v10_id} topTeam={team9} bottomTeam={team10} topSeed={9} bottomSeed={10}
                              onWinnerClick={handleWin} selectedWinnerId={picks[pi_9v10_id]}
                  ...
                  <div className="flex flex-col justify-center pt-4">
                      <div className="text-[7px] uppercase text-orange-400 mb-0.5 ml-1 text-center font-bold">For 8th Seed</div>
                      <BracketMatch 
                          matchId={pi_8th_seed_id} topTeam={loser7v8} bottomTeam={winner9v10} topSeed="L7" bottomSeed="W9"
                          onWinnerClick={handleWin} selectedWinnerId={picks[pi_8th_seed_id]}
  ...
  if (standings.West.length < 10) return <div className="text-white text-center mt-10">Please complete your Regular Season predictions first.</div>;
            ...
            <div className="transform scale-[0.65] sm:scale-[0.70] md:scale-[0.75] xl:scale-[0.80] origin-top flex flex-col items-center gap-6">
                ...
                <div className="flex flex-row items-center justify-center gap-0">
                    ...
                    <div className="flex flex-col items-center justify-center px-8 relative min-w-[200px] h-[500px] flex-shrink-0">
                        <img src="https://cdn.nba.com/logos/leagues/logo-nba.svg" className="h-24 opacity-10 absolute top-20" />
                        
                        <div className="mb-8 text-center">
                            <h1 className="text-3xl font-black text-white uppercase italic tracking-tighter mb-2 z-10">Finals</h1>
                            <div className="text-5xl drop-shadow-[0_0_30px_rgba(255,215,0,0.5)]">🏆</div>
                        </div>
                        ...
                        <div className={`transition-all duration-700 transform ${champion ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                            {champion && (
                                <div className="text-center">
                                    <div className="text-yellow-500 text-[9px] font-bold tracking-[0.3em] mb-1 uppercase">Champion</div>
                                    <div className="bg-gradient-to-r from-yellow-600 via-yellow-400 to-yellow-600 text-black px-6 py-2 rounded-xl shadow-[0_0_50px_rgba(234,179,8,0.4)] border border-yellow-200">
                                        <img src={champion.logo} className="w-12 h-12 object-contain mx-auto mb-1 drop-shadow-lg" />
                                        <div className="text-lg font-black uppercase tracking-tight">{champion.name}</div>
                                    </div>
                                </div>
                            )}
                        </div>
