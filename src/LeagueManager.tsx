	...
	return (
		<div className="bg-white/10 backdrop-blur-md p-4 rounded-xl border border-white/20 mb-8 w-full max-w-4xl shadow-xl">
			<div className="flex gap-2 border-b border-white/10 pb-4 mb-4 overflow-x-auto">
				<button onClick={() => setActiveTab('list')} className={`px-4 py-2 rounded-lg font-medium transition-all whitespace-nowrap ${activeTab === 'list' ? 'bg-white text-[#1D428A]' : 'text-white/70 hover:text-white hover:bg-white/10'}`}>My Leagues</button>
				<button onClick={() => setActiveTab('create')} className={`px-4 py-2 rounded-lg font-medium transition-all whitespace-nowrap ${activeTab === 'create' ? 'bg-white text-[#1D428A]' : 'text-white/70 hover:text-white hover:bg-white/10'}`}>Create New</button>
				<button onClick={() => setActiveTab('join')} className={`px-4 py-2 rounded-lg font-medium transition-all whitespace-nowrap ${activeTab === 'join' ? 'bg-white text-[#1D428A]' : 'text-white/70 hover:text-white hover:bg-white/10'}`}>Join via Code</button>
			</div>

			{activeTab === 'list' && (
				<div className="space-y-3">
					{myLeagues.length === 0 ? <p className="text-white/60 text-center py-2">You are not in any leagues yet.</p> : null}
			...
			{activeTab === 'create' && (
				<div className="flex gap-2">
					...
					/>
					<button onClick={createLeague} className="bg-green-500 hover:bg-green-600 text-white px-6 rounded-lg font-bold shadow-lg">Create</button>
				</div>
			)}
			...
			{activeTab === 'join' && (
				<div className="flex gap-2">
					...
					/>
					<button onClick={joinLeague} className="bg-blue-500 hover:bg-blue-600 text-white px-6 rounded-lg font-bold shadow-lg">Join</button>
				</div>
			)}
		</div>
	);
