...
export function BracketMatch({ 
	topTeam, bottomTeam, topSeed, bottomSeed, matchId, onWinnerClick, selectedWinnerId, officialWinnerId, reverse 
}: Props) {
	...
	if (officialWinnerId && selectedWinnerId) {
		if (officialWinnerId === selectedWinnerId) {
			...
			statusIcon = <div className="absolute -top-2 -right-2 bg-green-500 text-black text-[10px] w-5 h-5 flex items-center justify-center rounded-full font-bold z-20">✓</div>;
		} else {
			statusColor = 'border-red-500 opacity-60'; // wrong!
			statusIcon = <div className="absolute -top-2 -right-2 bg-red-500 text-white text-[10px] w-5 h-5 flex items-center justify-center rounded-full font-bold z-20">✕</div>;
		}
	}

	const TeamRow = ({ team, seed, isTop }: { team?: Team | null, seed?: number | string, isTop: boolean }) => {
			...
			>
				...
				{team ? (
					<>
						...
						<span className={`text-[9px] font-bold truncate leading-tight flex-1 ${isSelected ? 'text-white' : 'text-gray-400'}`}>{team.name}</span>
					</>
				) : (
	...
	return (
		<div className={`w-44 bg-[#1a1a1a] border rounded overflow-visible relative z-10 flex flex-col mb-2 transition-all duration-500 ${statusColor}`}>
			{statusIcon}
